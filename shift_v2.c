...
// Sorting the points in each dim
  ...
  for (i=0;i<npoints;i++){
	...
	for (b=0; b<nb_runs;b++){
		...
		if (!chosen) {// Checking that this is a real local minima and not just somewhere we get stuck because of our heuristic
		// ALso used if we can't use clean borders
			fprintf(stderr,"stuck");
			boom=false;
			i=0;
			j=0;
			while (i<npoints && !boom){
				if (is_in[i]< -0.5) { // The point was not already inside
				j=0;
					while (j<npoints && !boom){
						tempo=is_in[j];
						if (tempo>-0.5 && i!=j) { // Exchange candidate was inside
							insidei=true;
							insidej=true;
							for (h=0;h<dim;h++){ // Creating conditions for improvement, to restrict brute force
								if (fill){
									if (Orig_pointset[i][h]>top_bord[h]+epsilon/2) // epsilon because of the int imprecision
										insidei=false;
									if (Orig_pointset[j][h]>top_bord[h]+epsilon/2)
										insidej=false;
								}
								else{
									if (Orig_pointset[i][h]>=top_bord[h]-epsilon/2)
										insidei=false;
									if (Orig_pointset[j][h]>=top_bord[h]-epsilon/2)
										insidej=false;
								}
							}
							if ((fill && !insidei && insidej) ||(insidei && !fill && !insidej)){		// In the other cases, we won't be improving the box. Could be a good plateau but not one of our concerns.			
							
								for (f=0;f<kpoints;f++)
									memcpy(temp_subset[f],subset[f],dim*sizeof(double));
								memcpy(temp_subset[tempo],Orig_pointset[i],dim*sizeof(double));
								curr_disc = oydiscr(temp_subset, dim,kpoints, &lower);
								glob_bound=0;
								lower=0.0;
								globallower=0.0;
								...
								if (curr_disc<upper) {// Our replacement is good
									boom=true;
									nb_brute+=1;
									memcpy(subset[tempo],Orig_pointset[i],dim*sizeof(double));
									is_in[i]=tempo;
									is_in[j]=-1;
	...
	if (!chosen && !boom) // We've reached a local minima
		break;
	else{
		if (!chosen)
			fprintf(stderr,"free");
	...
	fprintf(stderr,"Nb calcu:%d, Final discre:%lf",nb_calc,upper);
	for (i=0;i<kpoints;i++)
...
int main(int argc, char **argv)
{
  ...
  for (nb_tries=0;nb_tries<1;nb_tries++){
  int rando1;
  int rando2;
  double *swap;

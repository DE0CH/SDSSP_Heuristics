...
usage() {
    echo "Usage: $0 executable input_file dimension values [-j num_cores]"
...
command_function() {
    local value=$1
    python3 ../match_index.py ${input_file} subset_${value}.txt /dev/null subset_complement_${value}.csv
    python3 ../extract_cvs.py subset_complement_${value}.csv subset_complement_${value}.txt
    SHIFT_TRIES=5000 ${executable} 3 $ subset_complement_${value}.txt 2>&1 | ts > log_subset_complement_${value}.txt
    python3 ../match_index.py "$csv_file" "subset_complement_${value}.txt" "subset_complement_${value}.csv" /dev/null 
